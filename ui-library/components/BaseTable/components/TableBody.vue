<template>
  <tbody class="base-table__body">
    ...
    >
      ...
      >
        <component
          ...
          @save="(value: any) => handleCellSave(row, column, value)"
    ...
    <tr v-if="data.length === 0" class="base-table__row base-table__row--empty">
      ...
      >
        <slot name="empty"> داده‌ای یافت نشد </slot>
      </td>
    </tr>
  </tbody>
</template>
...
type BuiltinCellType = keyof typeof cellRenderers;
...
function getCellRenderer(column: Column) {
  // اگر خود ستون یک renderer داده، همونو برگردون
  if (column.render) return column.render;

  // کلید معتبر بساز: اگر type تعریف نشده، برو روی 'string'
  const key = (column.type ?? "string") as BuiltinCellType;

  // و خیلی امن برگردون
  return cellRenderers[key] ?? cellRenderers.string;
}
...
function isEditing(row: any, column: Column): boolean {
  ...
  return (
    props.editingCell?.row === row && props.editingCell?.column === column.key
  );
}
